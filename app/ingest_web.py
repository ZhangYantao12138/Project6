...
# MD5 of the 511NY "No live camera feed" placeholder image
...
def fetch_active_cameras(db, limit=CAMERAS_PER_RUN, offset=0):
    # sort by camera_id for stable partitioning across parallel batches
    return list(
        db.cameras.find(
            {"status": "active", "image_url": {"$ne": None}},
            {"_id": 0},
        ).sort("camera_id", 1).skip(offset).limit(limit)
    )
...
def ingest_one(camera_doc, db, minio_client: Minio):
    ...
    # append Unix timestamp as cache-buster (matches 511ny.org behaviour)
    ...
    try:
        ...
        if "image" not in content_type:
            ...
            print(f"[WARN] {camera_id} cycle={ts_str} non-image response: {content_type}")
            return
        ...
        if hashlib.md5(image_bytes).hexdigest() == NO_FEED_MD5:
            ...
            print(f"[SKIP] {camera_id} cycle={ts_str} -> no live feed")
            return
...
def _run(offset: int = 0, limit: int = CAMERAS_PER_RUN):
    db = init_mongo()
    minio_client = Minio(
        MINIO_ENDPOINT,
    ...
    cameras = fetch_active_cameras(db, limit=limit, offset=offset)
    print(f"[BATCH] offset={offset} limit={limit} cameras={len(cameras)}")
    for cam in cameras:
...
def main():
    _run(offset=0, limit=CAMERAS_PER_RUN)


def main_batch(batch_config: dict):
    _run(offset=batch_config["offset"], limit=batch_config["limit"])
